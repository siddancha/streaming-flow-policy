...
class SFPUnetHybridImagePolicy(BaseImagePolicy):
    ...
    def streaming_predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        """
        ...
        naction_pred = self.streaming_get_traj(obs_dict) #[8, 56, 1, 2]
    ...
    def chunk_predict_action(self, obs_dict: Dict[str, torch.Tensor], prev_action = None) -> Dict[str, torch.Tensor]:
        """
        obs_dict: must include "obs" key
        result: must include "action" key
        """
        ...
        naction_pred = self.chunk_get_traj(global_cond, nobs, obs_dict, prev_action) #[8, 56, 1, 2]
        # print('naction_pred', naction_pred[:,0,...])
