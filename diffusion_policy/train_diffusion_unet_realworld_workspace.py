...
# multi-gpu support
import torch.distributed as dist #aded for two gpusAdd commentMore actions
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if (self.epoch % cfg.training.checkpoint_every) == 0:
                    # checkpointing
                    if cfg.checkpoint.save_last_ckpt and (not self.multi_gpu or is_main_process()):
                ...
                # end of epoch
                # log of last step is combined with validation and rollout
                if not self.multi_gpu or is_main_process(): wandb_run.log(step_log, step=self.global_step)
                json_logger.log(step_log)
                self.global_step += 1
                self.epoch += 1
